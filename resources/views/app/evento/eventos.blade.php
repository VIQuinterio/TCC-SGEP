...
<body>
            ...
            <form action="{{ route('app.evento.buscar') }}" method="GET" class="flex items-center">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" name="buscar" id="buscar" style="width: 300px;"
                        ...
                        placeholder="Procurar eventos. Exemplo: 2023-06-26" required>
                </div>                
            </form>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                <tr style="background-color: #0B3142; color:aliceblue;">
                    ...
                    <th scope="col" class="px-6 py-3">
                        Nome
                        <a href="{{ route('app.evento.index', ['sort' => 'nome', 'direction' => 'asc']) }}" class="sort-link">
                            <span class="sort-arrow sorts asc" data-sort="nome" data-direction="asc">&#8593;</span>
                        </a>
                        <a href="{{ route('app.evento.index', ['sort' => 'nome', 'direction' => 'desc']) }}" class="sort-link">
                            <span class="sort-arrow sorts desc" data-sort="nome" data-direction="desc">&#8595;</span>
                        </a> 
                    </th>
                    <th scope="col" class="px-6 py-3">
                        Descrição
                        <a href="{{ route('app.evento.index', ['sort' => 'descricao', 'direction' => 'asc']) }}" class="sort-link">
                            <span class="sort-arrow sorts asc" data-sort="descricao" data-direction="asc">&#8593;</span>
                        </a>
                        <a href="{{ route('app.evento.index', ['sort' => 'descricao', 'direction' => 'desc']) }}" class="sort-link">
                    ...
                    <th scope="col" class="px-6 py-3">
                        Unidade
                        <a href="{{ route('app.evento.index', ['sort' => 'unidade', 'direction' => 'asc']) }}" class="sort-link">
                            <span class="sort-arrow sorts asc" data-sort="unidade" data-direction="asc">&#8593;</span>
                        </a>
                        <a href="{{ route('app.evento.index', ['sort' => 'unidade', 'direction' => 'desc']) }}" class="sort-link">
                ...
                <tbody>
                    @if (isset($resultados_busca) && count($resultados_busca) > 0)
                        @foreach ($resultados_busca as $event)
                            <tr
                                ...
                                <td class="px-6 py-4 w-auto">
                                    ...
                                    <div class="flex items-center">
                                        <button type="button" data-modal-target="edit-modal{{$event->id_evento}}" data-modal-toggle="edit-modal{{$event->id_evento}}"
                                        ...
                                        <form
                                            action="{{ route('app.evento.excluir', ['id' => $event->id_evento]) }}"
                                            method="POST">
                                            @csrf
                            ...
                            <tr
                                ...
                                <td class="px-6 py-4 w-auto">
                                    ...
                                    <div class="flex items-center">                                                                                
                                        ...
                                        <form action="{{ route('app.evento.excluir', ['id' => $event->id_evento]) }}" method="POST">
                                            @csrf
                                            <input type="hidden" name="event_id" value="{{ $event->id_evento }}">
            ...
            @else
                {{ $list_event->appends(['sort' => request()->input('sort'), 'direction' => request()->input('direction')])->links('layouts.custom-pagination') }}
            @endif
        </div>
    </div>
    @if (session('success'))
    ...
    @section('titulo', 'Cadastrar evento')
        @section('form')
            <form class="space-y-4" action="{{ route('app.evento.cadastro') }}" method="POST">
                @csrf
