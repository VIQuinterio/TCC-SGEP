...
<body>
    ...
    <div>
        <div class="container mx-auto pt-32" style="padding-bottom: 20px">
            <div class="max-w-screen-lg mx-auto flex">
                        ...
                        <div class="relative h-50 overflow-hidden md:h-120 rounded-lg">
                            @foreach ($list_news as $index => $news)
                                <div class="{{ $index === 0 ? '' : 'hidden' }} duration-700 ease-in-out" data-carousel-item>
                                    <img src="{{ asset('storage/' . $news->im_capa) }}" class="absolute block w-full h-full object-cover -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="...">
                                    <div class="absolute inset-0 flex flex-col items-start justify-end text-white z-10 pb-10 pl-5">
                                        <h2 class="md:text-3xl text-3xl font-extrabold text-left uppercase" style="text-shadow: rgb(0, 0, 0) 1px 0 10px;">{{ $news->nm_titulo }}</h2>
                                        <div class="bg-blue-700 p-1.5">
                                            <p class="text-white text-sm text-left">{{ \Carbon\Carbon::parse($news->dt_noticia)->translatedFormat('j \d\e F \d\e Y') }}</p>
                                        </div>                                    
                                    </div>
                                </div>
                            @endforeach
                ...
                <div class="w-1/2 pl-8">
                        ...
                        @foreach ($other_news as $index => $noticia)
                            ...
                            @if ($index !== 0 && $index > 2)
                                <div class="border-b border:gray-100  p-4 news-item">
                                    ...
                                    <!-- Data da notícia -->
                                    <p class="text-gray-500 text-sm">{{ \Carbon\Carbon::parse($noticia->dt_noticia)->translatedFormat('j \d\e F \d\e Y') }}</p>
                                    <!-- Breve descrição ou resumo da notícia -->
                                    <p class="text-gray-700">{{ \Illuminate\Support\Str::limit(strip_tags($noticia->ds_conteudo), 100) }}</p>
                                    <!-- Link para a notícia completa -->
                                    <form action="{{ route('app.noticia.detalhes', ['id' => $noticia->id_noticia]) }}" method="POST">
                                        @csrf
                    ...
                    <div class="mt-4">
                        <button id="prev-btn" class="px-4 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-800">Anterior</button>
                        <button id="next-btn" class="px-4 py-2 bg-blue-700 text-white rounded-md hover:bg-blue-800">Próxima</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-1 p-25">
                <div id="eventos" class="flex-1 ml-64 p-2" style="display: flex; flex-direction: row; margin: 5px; flex-wrap: wrap; justify-content:center;">
                    <h1 class="flex items-center text-5xl font-extrabold dark:text-white m-10">Eventos da região</h1>
                </div>
                <div id="eventos" class="flex-1 ml-64 p-6"
            ...
            <div class="p-25">
                ...
                <div id="modalidades" class="flex-1 ml-64 p-6 sport-area">
            ...
            <div class="bg-1 p-25">
                <div id="unidades" class="flex-1 ml-64 p-2" style="display: flex; flex-direction: row; margin: 5px; flex-wrap: wrap; justify-content:center;">
